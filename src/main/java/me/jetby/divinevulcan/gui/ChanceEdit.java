...
public class ChanceEdit extends AdvancedGui {
    ...
    public ChanceEdit(Player player, String type, String inv, Items items, Main plugin) {
        ...
        onClose(event -> {
            saveChanges();
            Bukkit.getScheduler().runTaskLater(plugin, ()->{new InvEdit(player, type, items, plugin).open(player);}, 1L);
        });
    }
